...
2. Tanstack-query의 queryKey
   현재 모든 경우에 따른 데이터를 캐싱해 두고 있지만, 이는 사실 좋은 방법이 아니기도 합니다. 특히 사용자의 입력값까지 현재 `queryKey`로 사용하고 있는데 카테고리와 달리 입력값은 그 범위가 무한정이기 때문에 이 부분은 지양하는게 맞는것 같습니다.
